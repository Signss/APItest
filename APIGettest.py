...
class TuBoGetAPI(object):
    ...
    # 编辑室列表
    def edit_list(self, url):
        try:
            payload = {'type': compare_contants.EDIT_LIST_TYPE}
            headers = {
                'Authorization': self.Authorization
            }
            response = requests.get(self.domain + url, params=payload, headers=headers)
            ...
            try:
                ...
                if code != compare_contants.COMMON_CODE:
                    compare_content['code'] = code
                if len(r_dict.get('data')) < compare_contants.EDITLIST_DATA_LENGTH:
                    self.deal_lack(self.lack_response_err_file, url, len(r_dict.get('data')), response.status_code)
                elif len(compare_content) > compare_contants.LACK_NUM:
                    self.response_err_file.write(str(compare_content) + '\n')
                else:
    ...
    def prep(self, url):
        try:
            ...
            try:
                ...
                if code != compare_contants.COMMON_CODE:
                    compare_content['code'] = code
                if len(r_dict.get('data')) < compare_contants.PREP_DATA_LENGTH:
                    self.deal_lack(self.lack_response_err_file, url, len(r_dict.get('data')), response.status_code)
                elif len(compare_content) > compare_contants.LACK_NUM:
                    self.response_err_file.write(str(compare_content) + '\n')
                else:
    ...
    def user_subscribe(self, url):
        try:
            ...
            try:
                ...
                if code != compare_contants.COMMON_CODE:
                    compare_content['code'] = code
                if len(r_dict.get('data')) < compare_contants.USERSUBSCRIBE_DATA_LENGTH:
                    self.deal_lack(self.lack_response_err_file, url, len(r_dict.get('data')),
                                   response.status_code)
                elif len(compare_content) > compare_contants.LACK_NUM:
                    self.response_err_file.write(str(compare_content) + '\n')
    ...
    def share(self, url):
        try:
            payload = {'id': compare_contants.SHARE_ID, 'type': compare_contants.SHARE_TYPE}
            headers = {
                'Authorization': self.Authorization
            }
            response = requests.get(self.domain + url, params=payload, headers=headers)
            ...
            try:
                ...
                if len(r_dict.get('data')) < compare_contants.SHARE_DATA_LENGTH:
                    self.deal_lack(self.lack_response_err_file, url, len(r_dict.get('data')), response.status_code)
                elif len(compare_content) > compare_contants.LACK_NUM:
                    self.response_err_file.write(str(compare_content) + '\n')
                else:
    ...
    def banner(self, url):
        try:
            ...
            try:
                ...
                if len(r_dict.get('data')) < compare_contants.BANNER_DATA_LENGTH:
                    self.deal_lack(self.lack_response_err_file, url, len(r_dict.get('data')), response.status_code)
                elif len(compare_content) > compare_contants.LACK_NUM:
                    self.response_err_file.write(str(compare_content) + '\n')
                else:
